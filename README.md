...
除了直接引用 `convert.min.js` 动态覆写（源码位于 `src/`，由 CI 编译生成 `convert.js` 再压缩得到），你也可以使用仓库中预先生成好的 YAML 格式覆写——它们都放在 yamls/ 目录里，由 GitHub Actions 在每次推送后自动重新生成、覆盖。适用于诸如 Clash Verge 等不支持 JS 覆写的客户端和订阅转换服务。
...
- 源码位置：`src/*.ts`
- 产物位置：仓库根目录 `convert.js`（构建产物）与 `convert.min.js`（压缩产物）
